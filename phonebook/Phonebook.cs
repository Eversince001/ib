...
namespace phonebook
{
    public partial class Phonebook : Form
    {
        ...
        private int id;
        public Phonebook(auth back, ref List<List<string>> usrs, int id)
        {
            InitializeComponent();
            this.id = id;
            this.back = back;
            this.usrs = usrs;
            loglabel.Text += usrs[id][0];

            if (usrs[id][2] != "Администратор")
            {
        ...
        private void ChangeButton_Click(object sender, EventArgs e)
        {
            ...
            if (FIOTextBox.Text.Length != 0 && PhonenumberTextBox.Text.Length == 0)
            {
                user rec = Book.getUserByid(PhoneBookList.SelectedItems[0].Text);
        ...
        {
            Form reg = new registration(this, ref usrs, true);
            this.Hide();
            reg.Show();
        }
        ...
        {
            Form users = new usrs(this, ref usrs);
            this.Hide();
            users.Show();
        }
    }
